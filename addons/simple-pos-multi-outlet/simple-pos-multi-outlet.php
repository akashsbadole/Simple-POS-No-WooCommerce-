...
define( 'SMO_VERSION', '1.0.0' );

add_action( 'simple_pos_init', 'smo_boot' );
...
function smo_boot() {
	if ( ! class_exists( 'Simple_POS_Addons' ) ) {
		add_action(
			...
			function () {
				...
				esc_html_e( 'Simple POS — Multi-Outlet requires the free Simple POS plugin to be installed and active.', 'wp-pos-plugin' );
				echo '</p></div>';
			}
		);
		return;
	...
	add_filter(
		...
		function ( $addons ) {
			$addons[] = array(
				...
				'version'     => SMO_VERSION,
				'description' => __( 'Outlets with per-sale tagging and outlet-filtered sales history.', 'wp-pos-plugin' ),
			);
			return $addons;
		}
	);
	...
	add_action( 'admin_post_simple_pos_outlet_save', 'smo_handle_save' );
	add_action( 'admin_post_simple_pos_outlet_delete', 'smo_handle_delete' );
	add_action( 'admin_post_simple_pos_outlet_default', 'smo_handle_default' );
}

/**
 * Render the Outlets screen.
 */
function smo_render_page() {
	if ( ! current_user_can( 'manage_pos_products' ) ) {
		wp_die( esc_html__( 'You are not allowed to manage outlets.', 'wp-pos-plugin' ) );
	}
	SMO_Outlets::ensure_schema();
...
function smo_handle_save() {
	...
	if ( $id > 0 ) {
		SMO_Outlets::update_outlet( $id, $name, $addr );
	} else {
		SMO_Outlets::create_outlet( $name, $addr );
	}
 ...
 */
function smo_handle_default() {
	if ( ! current_user_can( 'manage_pos_products' ) ) {
		wp_die( esc_html__( 'You are not allowed to manage outlets.', 'wp-pos-plugin' ) );
	}
	check_admin_referer( 'simple_pos_outlet_default' );

	SMO_Outlets::set_default_outlet_id( isset( $_POST['outlet_id'] ) ? (int) $_POST['outlet_id'] : 0 );
