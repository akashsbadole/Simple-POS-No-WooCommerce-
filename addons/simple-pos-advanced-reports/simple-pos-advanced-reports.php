 ...
 * Version:     1.0.0
 * Author:      Simple POS
 * Text Domain: simple-pos-adv-reports
 */
...
function spar_boot() {
	if ( ! class_exists( 'Simple_POS_Addons' ) ) {
		add_action( 'admin_notices', function () {
			...
			esc_html_e( 'Simple POS — Advanced Reports requires the free Simple POS plugin to be installed and active.', 'simple-pos-adv-reports' );
			echo '</p></div>';
		} );
		return;
	}
	...
	add_filter( 'simple_pos_registered_addons', function ( $addons ) {
		$addons[] = array(
			...
			'name'        => __( 'Advanced Reports', 'simple-pos-adv-reports' ),
			'version'     => SPAR_VERSION,
			'description' => __( 'X/Z summaries, hourly heatmap, payment mix, product mix, tax summary and CSV exports.', 'simple-pos-adv-reports' ),
		);
		return $addons;
	} );
	...
	add_action( 'admin_menu', function () {
		add_submenu_page(
			'simple-pos-terminal',
			__( 'Reports Pro', 'simple-pos-adv-reports' ),
			__( 'Reports Pro', 'simple-pos-adv-reports' ),
			'view_pos_reports',
...
function spar_render_page() {
	if ( ! current_user_can( 'view_pos_reports' ) ) {
		wp_die( esc_html__( 'You are not allowed to view reports.', 'simple-pos-adv-reports' ) );
	}
...
function spar_handle_export() {
	if ( ! current_user_can( 'view_pos_reports' ) ) {
		wp_die( esc_html__( 'You are not allowed to export reports.', 'simple-pos-adv-reports' ) );
	}
	...
	if ( is_wp_error( $data ) || null === $data ) {
		wp_die( esc_html__( 'Unknown report type.', 'simple-pos-adv-reports' ) );
	}
