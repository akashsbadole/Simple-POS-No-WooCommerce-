...
class Simple_POS_CSV {
	...
	public static function export_products() {
		$all = Simple_POS_Products::get_products(array('per_page'=>10000,'page'=>1,'status'=>'any'));
	...
	public static function import_products($file_path) {
		if (!file_exists($file_path)) return new WP_Error('pos_file_missing',__('CSV file missing.','simple-pos'));
		$handle=fopen($file_path,'r');
		if(!$handle) return new WP_Error('pos_file_error',__('Could not open CSV.','simple-pos'));
		$header=fgetcsv($handle);
		if(!$header) { fclose($handle); return new WP_Error('pos_invalid_csv',__('Empty CSV.','simple-pos'));}
		$header = array_map('strtolower', array_map('trim',$header));
		$required=array('name','price');
		foreach($required as $r){ if(!in_array($r,$header,true)){ fclose($handle); return new WP_Error('pos_invalid_csv', sprintf(__('Missing column: %s','simple-pos'),$r)); } }
		...
		while(($row=fgetcsv($handle))!==false){
			...
			if(!empty($data['id'])){
				...
				if(is_wp_error($res)) $errors[]="Row $rownum: ".$res->get_error_message();
				else $imported++;
			} else {
				// check duplicate sku
	...
	public static function export_sales($date_from='',$date_to=''){
		$args=array('per_page'=>10000,'page'=>1);
