...
class Simple_POS_Variants {
	...
	public static function get_variants( $parent_id, $status='any' ) {
		...
		if ('any'!==$status) { $where.=' AND status=%s'; $params[]=$status; }
	...
	public static function create_variant( $parent_id, $data ) {
		...
		if ( ! $parent ) return new WP_Error('pos_not_found',__('Parent product not found.','simple-pos'));
		$clean = self::sanitize($data);
		if (is_wp_error($clean)) return $clean;
		if (!empty($clean['sku']) && self::sku_exists($clean['sku'])) return new WP_Error('pos_duplicate_sku',__('SKU already used.','simple-pos'));
		if (!empty($clean['barcode']) && self::barcode_exists($clean['barcode'])) return new WP_Error('pos_duplicate_barcode',__('Barcode already used.','simple-pos'));
		$clean['parent_product_id']=$parent_id;
		$clean['created_at']=current_time('mysql');
		$clean['updated_at']=current_time('mysql');
		$inserted=$wpdb->insert($table,$clean);
		if(false===$inserted) return new WP_Error('pos_db_error',__('Could not create variant.','simple-pos'));
	...
	public static function update_variant( $id, $data ) {
		...
		if(!$existing) return new WP_Error('pos_not_found',__('Variant not found.','simple-pos'));
		$clean=self::sanitize($data,$existing);
		if(is_wp_error($clean)) return $clean;
		if(!empty($clean['sku']) && self::sku_exists($clean['sku'],$id)) return new WP_Error('pos_duplicate_sku',__('SKU already used.','simple-pos'));
		if(!empty($clean['barcode']) && self::barcode_exists($clean['barcode'],$id)) return new WP_Error('pos_duplicate_barcode',__('Barcode already used.','simple-pos'));
	...
	public static function adjust_stock($variant_id,$delta,$reason='adjustment',$reference_id=null,$note=''){
		...
		if(!$variant) return new WP_Error('pos_not_found',__('Variant not found.','simple-pos'));
		if(!$variant->track_stock) return true;
		$settings=Simple_POS_Settings::get_all();
		$new_qty=(int)$variant->stock_qty + (int)$delta;
		if($new_qty<0 && empty($settings['allow_negative_stock'])) return new WP_Error('pos_insufficient_stock', sprintf(__('Not enough stock for variant "%s".','simple-pos'), self::variant_label($variant)));
		$table=Simple_POS_DB::table('product_variants');
		$wpdb->update($table,array('stock_qty'=>$new_qty,'updated_at'=>current_time('mysql')),array('id'=>$variant_id));
	...
	public static function variant_label($v){
		...
		if(empty($attrs) || !is_array($attrs)) return 'Variant #'.$v->id;
		return implode(' / ', array_map(function($k,$val){ return $k.': '.$val; }, array_keys($attrs), $attrs));
	}
	...
	private static function sanitize($data,$existing=null){
		...
		if($price!==null && $price<0) return new WP_Error('pos_invalid_input',__('Price cannot be negative.','simple-pos'));
