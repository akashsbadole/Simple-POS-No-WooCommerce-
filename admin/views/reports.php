...
<div class="wrap simple-pos-wrap">
	<div class="simple-pos-page-header">
		<h1><?php esc_html_e( 'Reports', 'wp-pos-plugin' ); ?></h1>
	</div>
	...
	<div class="simple-pos-card simple-pos-filter-bar">
		<form method="get" class="simple-pos-filters">
			...
			<div class="simple-pos-filter-field">
				<label for="rep-range"><?php esc_html_e( 'Range', 'wp-pos-plugin' ); ?></label>
				<select id="rep-range" name="range" onchange="if(this.value!=='custom'){this.form.submit()}">
					<option value="today" <?php selected( $range, 'today' ); ?>><?php esc_html_e( 'Today', 'wp-pos-plugin' ); ?></option>
					<option value="7days" <?php selected( $range, '7days' ); ?>><?php esc_html_e( 'Last 7 days', 'wp-pos-plugin' ); ?></option>
					<option value="30days" <?php selected( $range, '30days' ); ?>><?php esc_html_e( 'Last 30 days', 'wp-pos-plugin' ); ?></option>
					<option value="month" <?php selected( $range, 'month' ); ?>><?php esc_html_e( 'This month', 'wp-pos-plugin' ); ?></option>
					<option value="custom" <?php selected( $range, 'custom' ); ?>><?php esc_html_e( 'Custom range', 'wp-pos-plugin' ); ?></option>
				</select>
			</div>
			<?php if ( 'custom' === $range ) : ?>
				<div class="simple-pos-filter-field">
					<label for="rep-from"><?php esc_html_e( 'From', 'wp-pos-plugin' ); ?></label>
					<input id="rep-from" type="date" name="date_from" value="<?php echo esc_attr( $date_from ); ?>" />
				</div>
				<div class="simple-pos-filter-field">
					<label for="rep-to"><?php esc_html_e( 'To', 'wp-pos-plugin' ); ?></label>
					<input id="rep-to" type="date" name="date_to" value="<?php echo esc_attr( $date_to ); ?>" />
				</div>
				<div class="simple-pos-filter-actions">
					<button type="submit" class="button button-primary"><?php esc_html_e( 'Apply', 'wp-pos-plugin' ); ?></button>
				</div>
			<?php endif; ?>
		</form>
	</div>
	...
	<div class="simple-pos-stat-cards">
		<div class="simple-pos-stat-card">
			<span class="simple-pos-stat-label"><?php esc_html_e( 'Revenue', 'wp-pos-plugin' ); ?></span>
			<span class="simple-pos-stat-value"><?php echo esc_html( Simple_POS_DB::format_currency( $summary['revenue'] ) ); ?></span>
		</div>
		<div class="simple-pos-stat-card">
			<span class="simple-pos-stat-label"><?php esc_html_e( 'Sales', 'wp-pos-plugin' ); ?></span>
	...
	<div class="simple-pos-card">
		<h2 class="simple-pos-section-title" style="margin:0 0 10px"><?php esc_html_e( 'Revenue by Day', 'wp-pos-plugin' ); ?></h2>
		<?php if ( empty( $by_day ) ) : ?>
			<p class="simple-pos-muted"><?php esc_html_e( 'No sales in this range.', 'wp-pos-plugin' ); ?></p>
		<?php else : ?>
	...
	<div class="simple-pos-columns">
		<div class="simple-pos-col-main">
			<div class="simple-pos-card simple-pos-table-card">
				<h2 class="simple-pos-section-title" style="margin:0;padding:14px 14px 0"><?php esc_html_e( 'Top Products', 'wp-pos-plugin' ); ?></h2>
				<table class="wp-list-table widefat striped simple-pos-table" style="margin-top:10px">
					<thead>
						<tr>
							<th><?php esc_html_e( 'Product', 'wp-pos-plugin' ); ?></th>
							<th class="num"><?php esc_html_e( 'Qty Sold', 'wp-pos-plugin' ); ?></th>
							<th class="num"><?php esc_html_e( 'Revenue', 'wp-pos-plugin' ); ?></th>
						</tr>
					</thead>
					<tbody>
						<?php if ( empty( $top_products ) ) : ?>
							<tr><td colspan="3" class="simple-pos-empty"><?php esc_html_e( 'No sales in this range.', 'wp-pos-plugin' ); ?></td></tr>
						<?php else : ?>
		...
		<div class="simple-pos-col-side">
			<div class="postbox simple-pos-form-card">
				<h2 class="hndle"><span><?php esc_html_e( 'By Cashier', 'wp-pos-plugin' ); ?></span></h2>
				<div class="inside">
